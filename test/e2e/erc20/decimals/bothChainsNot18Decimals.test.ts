...
describe("E2E ERC20 - Two EVM Chains both with decimal places != 18", () => {
    ...
    const originDepositAmount = parseUnits("14", originDecimalPlaces);
    const destinationDepositAmount = parseUnits("14", destinationDecimalPlaces);
    const relayerConvertedAmount = parseUnits("14", bridgeDefaultDecimalPlaces);
    ...
    it(`E2E: depositAmount of Origin ERC20 owned by depositAddress to Destination ERC20
        owned by recipient.account!.address and back again`, async () => {
        ...
        await expect(originDepositTx).to.emit(OriginBridgeInstance, "Depost").withArgs(
          destinationDomainID,
          originResourceID.toLowerCase(),
          expectedDepositNonce,
          originDepositData.toLowerCase(),
          toHex(relayerConvertedAmount, {size:32})
        );

        // this mocks depositProposal data for executing on
        // destination chain which is returned from relayers
        const originDepositProposalData = await createDepositProposalDataFromHandlerResponse(
        ...
        // Assert ERC20 balance was transferred to recipient.account!.address
        ...
        assert.strictEqual(
          ...
          "originDepositAmount wasn't transferred to recipient.account!.address"
        );
        ...
        // recipient.account!.address makes a deposit of the received depositAmount
        ...
        await expect(
          OriginBridgeInstance.write.executeProposal([
            ...
            {account: originRelayer1Address.account}
          )
        ).not.to.be.reverted;

        // Assert ERC20 balance was transferred from recipient.account!.address
        recipientBalance = await DestinationERC20MintableInstance.read.balanceOf([
          recipient.account!.address
        ]);
        assert.strictEqual(recipientBalance.toString(), "0");
